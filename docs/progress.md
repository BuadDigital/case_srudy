...
- **`@failures/mfe`** — إدارة التعذرات (`/failures`), `FailureReportForm`, repository port + localStorage (API stub in `api-client/failures.ts`).
...
| `evalFailureRecords` (`FAILURES_STORAGE_KEY`) | تعذر records — `@failures/mfe` localStorage repository |
