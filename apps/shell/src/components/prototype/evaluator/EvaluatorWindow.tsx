...
import { inspectionGateForAppraisal } from "@/lib/prototype/evaluator/evaluator-inspection-gate";
import { cacheEvaluatorReport, getCachedEvaluatorReport, openEvaluatorReportPreview, } from "@/lib/prototype/evaluator/evaluator-report-attachments";
import { getOrCreateEvaluatorDraft, isEvaluatorFormLocked, updateEvaluatorDraft, } from "@/lib/prototype/evaluator/evaluator-submission-storage";
import { evaluatorStatusLabel, formatEvaluatorPriceDisplay, } from "@/lib/prototype/evaluator/evaluator-window-data";
import { firstEvaluatorError, validateEvaluatorSubmission, type EvaluatorValidationErrors, } from "@/lib/prototype/evaluator/evaluator-validation";
import { finalizeAppraiserSubmission } from "@/lib/prototype/evaluator/finalize-appraiser-submission";
...
export function EvaluatorWindow({
  ...
  const [reportName, setReportName] = useState<string | null>(() => {
    ...
    return cached?.fileName ?? draft.reportFileName;
  });
  ...
  const persistDraft = useCallback(
    (
      ...
      if (locked) return;
      const updated = updateEvaluatorDraft(task.id, patch);
  ...
  const submit = useCallback(async (): Promise<boolean> => {
    ...
    setFormError(null);
    const submitted = await finalizeAppraiserSubmission(task.id);
  ...
  async function onReportSelected(file: File | null) {
    ...
    persistDraft({ reportFileName: file.name });
    setFieldErrors((prev) => {
      const next = { ...prev };
  ...
  return (
    <div className="evaluator-window">
      ...
      <section className="evaluator-upload-card">
        <header className="evaluator-upload-card__hd">
          <h3 className="evaluator-upload-card__title">رفع التقييم</h3>
          <p className="evaluator-upload-card__sub">
            تقرير المقياس وسعر العقار — يُعرض للأخصائي للاسترشاد فقط
          </p>
        </header>

        <div className="evaluator-upload-card__body">
          <div
